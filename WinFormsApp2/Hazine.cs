public class Hazine : Engel
{
    public Hazine(int genislik, int yukseklik, Control.ControlCollection controls) : base(genislik, yukseklik, controls)
    {
    }


    public List<Point> treasureLocations = new List<Point>();
    public override void PlaceObject()
    {
        ...
        if (!collisionManager.IsCollision(occupiedSquaresForCurrentObject))
        {
            ...
            // Hazinenin konumunu listeye ekle
            treasureLocations.Add(new Point(startX, startY));
        }
        else
        {
